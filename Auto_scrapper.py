...
url="https://www.google.com/maps/search/pharmacies+near+me/@12.9160145,77.5242675,14z/data=!3m1!4b1"
options = Options()
options.add_argument("--headless") # Runs Chrome in headless mode.
options.add_argument("--lang=en")
options.add_argument('--no-sandbox') # Bypass OS security model
options.binary_location='C:/Users/CS-9/AppData/Local/Google/Chrome SxS/Application/chrome.exe'
    ...
    with open(file_name, 'w', newline='') as csvfile:
        ...
        for each in sent_data:
            try:
                ...
                rating=each.find('span',attrs={'class':'rs9iHBFJiiu__rating'}).text.encode(encoding="utf-8",errors='ignore').decode('utf-8')
                # no_review=each.find('span',attrs={'class':'section-result-num-ratings'}).text.encode(encoding="utf-8",errors='ignore').decode('utf-8')
                no_review=each.find('span',attrs={'class':'rs9iHBFJiiu__reviews'}).text.encode(encoding="utf-8",errors='ignore').decode('utf-8')
